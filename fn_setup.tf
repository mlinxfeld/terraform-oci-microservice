resource "null_resource" "FoggyKitchenSetupATPFnPush2OCIR" {
  ...
  #provisioner "local-exec" {
  #  command = "image=$(docker images | grep setupatpfn | awk -F ' ' '{print $3}') ; docker rmi -f $image"
...
resource "null_resource" "FoggyKitchenCustomerFnPush2OCIR" {
  ...
  #provisioner "local-exec" {
  #  command = "image=$(docker images | grep customersfn | awk -F ' ' '{print $3}') ; docker rmi -f $image"
